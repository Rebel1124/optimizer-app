...
with header:
    ...
    #col11.markdown("<h1 style='text-align: left; color: Purple; padding-left: 0px'>Arrow-Up Capital</h1>", unsafe_allow_html=True)
    ...
    st.markdown("<h2 style='text-align: left; color: teal; padding-left: 40px; font-size: 50px'>Portfolio Optimizer</h2>", unsafe_allow_html=True)
    
    st.markdown(" ")
    #col12.title('Arrow-Up Capital')
...
########## write function ###########################
#@st.cache(allow_output_mutation=True)
#def figure1(stocks, start_date, end_date, out_sample_days_held_back):
...
bm = get_data(benchmark, start_date, end_date)
...
def beta(historic_daily_returns):
    ...
    fig8 = px.scatter(historic_daily_returns, x='BM_Returns', y='Portfolio_returns', 
                      #title='Daily Returns Optimal Portfolio vs. Benchmark'
                      )
...
#data = {'Returns':p_ret, 'Volatility':p_vol}
#
#for counter, symbol in enumerate(selected_stocks):
#    data[symbol+' weight'] = [w[counter] for w in p_weights]
#
## Create portfolios dataframe to hold the portfolio weights of stocks, and portfolio return, volatility and sharpe ratio
#
...
#opt_port_df = pd.DataFrame(data={'Stocks': selected_stocks, 'Weight': optimal_risky_port[2:-1].values})
#
#fig9 = px.pie(opt_port_df, values='Weight', names='Stocks',
#            title='Optimal Portfolio Stock Weighting',
#            template=theme
#            )
